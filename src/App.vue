...
const checkScreen = () => {
  isMobile.value = window.innerWidth <= 768; // 768px — надежный рубеж мобильный/планшет
};
...
const translations = reactive({
  en: {
    ...
    galleryTitle: "Favorite arts",
    gallerySubtitle: "Last update 15.09.2026",
  ...
  ru: {
    ...
    galleryTitle: "Любимые арты",
    gallerySubtitle: "Обновлено 15.09.2026",
...
const mods = [
  {
    ...
    description: {
      en: 'Replaces the egg in the hands of Rennala with a melon.',
      ru: 'Заменяет яйцо в руках Ренналы на сочную дыньку'
    }
  },
  {
    title: 'Watermelon',
    ...
    description: {
      en: 'Replaces the egg in the hands of Rennala with a watermelon',
      ru: 'Заменяет яйцо в руках Ренналы на арбуз лооол'
    }
  },
  {
    ...
    description: {
      en: 'Sliders settings for Frieren character from anime "Frieren beyond journey\'s end".',
      ru: 'Настройки ползунков для персонажа Фрирен из аниме «Провожающая в последний путь Фрирен».'
    }
  },
  {
    ...
    description: {
      en: 'This mod adds the song "Die Alone" by Zetsubo Extreme 7★',
      ru: 'Этот мод добавляет песню «Die Alone» от Zetsubo Extreme 7★'
    }
  }
]
...
onMounted(async () => {
  // 1. Инициализация экрана и языка
  checkScreen();
  window.addEventListener('resize', checkScreen);
  currentLang.value = detectLanguage();
  await nextTick();
  // 2. Логика точек навигации (УЛУЧШЕННАЯ, работает с секциями любой длины)
  if (isMobile) {
    const wrapper = document.querySelector('.mobile-swiper-wrapper');
    const sections = document.querySelectorAll('.mobile-section');
    const dots = document.querySelectorAll('.swiper-dots .dot');
    const indicator = document.querySelector('.swiper-dots-indicator');

    if (wrapper && indicator && dots.length > 0) {
      let isManualScroll = false;

      const moveIndicator = (targetDot) => {
        if (!indicator || !targetDot) return;
        indicator.style.transform = `translateY(${targetDot.offsetTop}px)`;
      };

      // Функция, которая вычисляет активную секцию по позиции скролла
      const handleScroll = () => {
        if (isManualScroll) return; // Не мешаем анимации при клике

        const scrollTop = wrapper.scrollTop;
        const wrapperHeight = wrapper.clientHeight;
        let activeId = sections[0].id; // По умолчанию первая
        ...
        for (let i = 0; i < sections.length; i++) {
          ...
          // Проверяем, находится ли середина экрана внутри этой секции
          const viewportMiddle = scrollTop + (wrapperHeight / 2);
          ...
          // Страховка для самого низа длинной последней секции
          if (i === sections.length - 1 && (scrollTop + wrapperHeight) >= sectionBottom - 50) {
            activeId = section.id;
          }
        }

        const activeDot = document.querySelector(`.swiper-dots .dot[data-target="${activeId}"]`);
        if (activeDot) moveIndicator(activeDot);
      };

      // Слушаем скролл (passive: true для максимальной производительности)
      ...
      dots.forEach(dot => {
        dot.addEventListener('click', (e) => {
          ...
          // 1. Мгновенно двигаем индикатор
          moveIndicator(dot);

          // 2. Блокируем реакцию на скролл, чтобы не было дерганий
          isManualScroll = true;
          ...
          if (targetSection) {
            ...
            // 3. Снимаем блокировку после завершения анимации скролла (~600мс)
            setTimeout(() => {
              isManualScroll = false;
              handleScroll(); // Финальная синхронизация
            }, 600);
          }
        });
      });
  ...
  // 3. Остальная ваша логика (галерея, аватар, обложка)
  ...
  preload(coverUrl, coverLoaded)
  preload(avatarUrl, avatarLoaded)

});

function preload(url, flag) {
  const img = new Image()
  img.src = url
  img.onload = () => (flag.value = true)
  img.onerror = () => (flag.value = true)
  // если картинка уже в кеше — onload может не сработать
  if (img.complete && img.naturalWidth > 0) flag.value = true
}
...
const openGalleryModal = (imgSrc) => {
  ...
  setTimeout(() => {
    ...
    if (modalEl) {
      const modal = window.bootstrap ? new window.bootstrap.Modal(modalEl) : new Modal(modalEl)
      modal.show()
    }
  }, 50)
}

// "Предоставляем" функцию всем дочерним компонентам
provide('openGalleryModal', openGalleryModal)
...
<template>
  <div class="app">
      ...
      <section id="mob-links" class=" mobile-section ">
        <div class="contglass p-4 mx-4 my-2">
          ...
          <div class="hero-cover-layout mb-2">
            ...
            <!-- фон -->
            <div class="hero-cover-bg" :style="{ backgroundImage: `url(${coverUrl})` }"
          ...
          <div>
            ...
            <div class="social-row justify-content-center">
              ...
              >
                <svg :fill="s.fill" :width="s.svgwidth" :height="s.svgheight" :viewBox="s.viewBox"
                     xmlns="http://www.w3.org/2000/svg" v-html="s.svg"/>
                {{ s.label }}
              </a>

            </div>
          </div>
          <!-- черточка чтобы разделить это говно -->
    ...
    <main v-else class="container p-4 desktop-layout">
      ...
      <section id="gallery" class="contglass p-4 mt-4 mb-4 ">
        <div class="d-flex justify-content-between align-items-center mb-3">
      ...
      <!-- секция с модами -->
      <section id="mods" class="contglass p-4 mt-4 ">
    ...
    <!-- ═══ ТОЧКИ НАВИГАЦИИ (Только для мобильной версии) ═══ -->
    <nav v-if="isMobile" class="swiper-dots">
      <!-- Этот элемент будет плавно скользить между точками -->
      <div class="swiper-dots-indicator"></div>

      <a href="#mob-links" class="dot" data-target="mob-links"></a>
      <a href="#mob-gallery" class="dot" data-target="mob-gallery"></a>
      <a href="#mob-tgslider" class="dot" data-target="mob-tgslider"></a>
      <a href="#mob-mods" class="dot" data-target="mob-mods"></a>
    </nav>

    <!-- Модалка для галереи (одна картинка) -->
    <div
        class="modal fade"
        id="galleryModal"
        tabindex="-1"
    ...
    >
      ...
      <div class="modal-dialog modal-dialog-centered modal-xl">
        <div class="modal-content card-glass"
          ...
          <!-- Тело с одной картинкой, отцентрованной по центру -->
...
.gallery-modal-img {
  ...
  max-height: 85vh; /* Не выше 85% экрана */
  width: auto;
  height: auto;
...
.swiper-dots .dot {
  ...
  cursor: pointer;
  position: relative;
  z-index: 2; /* Поверх индикатора */
}

/* Скользящий индикатор (активная точка) */
.swiper-dots-indicator {
  ...
  border-radius: 50%;
  background: #2AABEE;
  box-shadow: 0 0 10px rgba(42, 171, 238, 0.6);
  /* cubic-bezier дает приятный "пружинящий" эффект при остановке */
  transition: transform 0.4s cubic-bezier(0.34, 1.56, 0.64, 1);
  z-index: 1; /* Под обычными точками */
  left: 0;
  top: 0;
}

/* ═══ ДЕСКТОП (Оставляем как есть, никаких медиа-запросов не нужно) ═══ */
.desktop-layout {
  /* Ваши стандартные стили работают по умолчанию */
}
